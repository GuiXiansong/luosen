...
<div class="container" style="width: 90%;">
    <br/>
    <div>
        <h3 align="center"><a href="<%=basePath%>/merchandising/toAddMerchandising.do">添加</a></h3>
    </div>
        ...
        <c:if test="${!empty requestScope.pagemsg}">
            <table class="table table-striped table-hover" id="countTable">
                ...
                <tr>
                    ...
                    <th style="text-align:center;">销售员编号</th>
                    <th style="text-align:center;">销售数量/重量</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <c:if test="${data.id!=null}">
                                <button type="button" onclick="return toModifyMerchandising(${data.id});"
                                        class="layui-btn layui-btn-sm layui-btn-danger">修改
                                </button>
                                <button type="button" onclick="return deleteMerchandising(${data.id});"
...
<table border="0" cellspacing="0" cellpadding="0" width="900px">
    <tr>
       ...
       <c:if test="${requestScope.pagemsg.currPage gt 1}">
           <a href="${pageContext.request.contextPath }/merchandising/findMerchandisingByStoreId.do?currentPage=1">[首页]</a>
           <a href="${pageContext.request.contextPath }/merchandising/findMerchandisingByStoreId.do?currentPage=${requestScope.pagemsg.currPage-1}">[上一页]</a>
       </c:if>
       <c:if test="${requestScope.pagemsg.currPage != requestScope.pagemsg.totalPage}">
           <a href="${pageContext.request.contextPath }/merchandising/findMerchandisingByStoreId.do?currentPage=${requestScope.pagemsg.currPage+1}">[下一页]</a>
           <a href="${pageContext.request.contextPath }/merchandising/findMerchandisingByStoreId.do?currentPage=${requestScope.pagemsg.totalPage}">[尾页]</a>
       </c:if>
   </span>
        </td>
    </tr>
...
<script type="text/javascript">
    //删除员工信息
    function deleteMerchandising(id) {
        layer.open({
            title: '警告信息',
            content: '你确定要删除该条信息？',
            btn: ['确定', '取消'],
            btn1: function (index, layero) {
                $.ajax({
                    url: '<%=basePath%>/merchandising/deleteMerchandising.do',
                    type: 'POST',
                    data: {id: id},
    ...
    //修改商品销售信息
    function toModifyMerchandising(id) {
        layer.open({
            title: '警告信息',
            content: '你确定要修改该条信息？',
            btn: ['确定', '取消'],
            btn1: function (index, layero) {
                $.ajax({
                    url: '<%=basePath%>/merchandising/toModifyMerchandising.do',
                    type: 'POST',
                    data: {id: id},
                    success: function (data) {
                        layer.open({
    ...
    $("#contractType").change(function () {
        ...
        selectMerchandisingInfoById(opval);
    })

    function selectContractType() {
        $('#contractType').empty();//先清空避免重复查询，此段也可省略
        $.ajax({
            url: '<%=basePath%>/merchandising/selectAllmerchandising.do',//后台的url地址
            type: 'post',
            success: function (data) {
    ...
    //选择商品销售信息
    function selectMerchandisingInfoById(id) {
        $.ajax({
            url: '<%=basePath%>/merchandising/selectMerchandisingInfoById.do',
            type: 'POST',
            data: {id: id},
            success: function (data) {
                layer.open({
